...
export async function getPriceIdForActiveUser(email: string) {
    ...
    const query = await sql`SELECT price_id FROM users WHERE email = ${email} AND status = 'aktif'`
...
export async function hasACtivePlan(email: string) {
    ...
    const query = await sql`SELECT price_id, status FROM users WHERE email = ${email} AND status = 'aktif' AND price_id IS NOT NULL`
